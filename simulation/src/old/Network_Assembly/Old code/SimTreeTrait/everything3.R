...
make.pa.mat <- function(traits) {
	...
	trait.narrow.abund <- trait.narrow*abund	
	
	p.trait.narrow <- trait.narrow.abund/sum(apply(trait.narrow.abund,1,sum))
	
	narrow.fill <- round(sum(trait.narrow.abund))
	
	trait.narrow.samp <- matrix(rmultinom(1, narrow.fill, prob=as.vector(p.trait.narrow)), nrow=nrow(p.trait.narrow))
	...
	trait.wide.abund <- trait.wide*abund	
	
	p.trait.wide <- trait.wide.abund/sum(apply(trait.wide.abund,1,sum))
	
	wide.fill <- round(sum(trait.wide.abund))
	
	trait.wide.samp <- matrix(rmultinom(1, wide.fill, prob=as.vector(p.trait.wide)), nrow=nrow(p.trait.wide))
	...
	trait.comp.abund <- trait.comp*abund	
	
	p.trait.comp <- trait.comp.abund/sum(apply(trait.comp.abund,1,sum))
	
	comp.fill <- round(sum(trait.comp.abund))
	
	trait.comp.samp <- matrix(rmultinom(1, comp.fill, prob=as.vector(p.trait.comp)), nrow=nrow(p.trait.comp))
	...
	trait.bar.abund <- trait.bar*abund	
	
	p.trait.bar <- trait.bar.abund/sum(apply(trait.bar.abund,1,sum))
	
	bar.fill <- round(sum(trait.bar.abund))
	
	trait.bar.samp <- matrix(rmultinom(1, comp.fill, prob=as.vector(p.trait.bar)), nrow=nrow(p.trait.bar))
	
	## equal probability of interating, matrix of 1s
	
	neutral <- matrix(1, nrow=nplant, ncol=npol)	
	
	neutral.abund <- neutral*abund	
	
	p.neutral <- neutral.abund/sum(apply(neutral.abund,1,sum))
	
	neutral.fill <- round(sum(neutral.abund))
	
	neutral.samp <- matrix(rmultinom(1, neutral.fill, prob=as.vector(p.neutral)), nrow=nrow(p.neutral))
...
### group together funmamental p matrices 
	...
	realized <- list(
		trait.comp.samp = trait.comp.samp, 
	 	trait.bar.samp = trait.bar.samp,
	 	neutral.samp = neutral.samp, 
	 	trait.narrow.samp = trait.narrow.samp,
	 	trait.wide.samp = trait.wide.samp)
	
	return(c(fundamental, ecological, realized))
}
...
link.rule <- function(traits) {
	...
	out.metrics <- lapply(interact,network.metrics, N=100)
