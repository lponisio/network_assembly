load.fix <- function(files, paths, nrule, nmat, ntopo){
  ...
  data <- cbind(data, mean.abund, sd.abund, w.evo)
  ...
  colnames(data)[colnames(data) == "vars"] <- c('a.var', 'p.var')
  colnames(data)[colnames(data) == "means"] <- c('a.mean', 'p.mean')
  colnames(data)[colnames(data) == "kt"] <- c('a.kt', 'p.kt')
  colnames(data)[colnames(data) == "skew"] <- c('a.skew', 'p.skew')
  colnames(data)[colnames(data) == "exponential"] <- c('a.exp', 'p.exp')
  colnames(data)[colnames(data) == "power law"] <- c('a.pl', 'p.pl')
  colnames(data)[colnames(data) == "truncated power law"] <- c('a.tpl', 'p.tpl')
