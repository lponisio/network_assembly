## builds off SimTreeTrait_samp_mod. Using lognormal instead of
## poisson log normal to draw abundances.
...
make.pa.mat <- function(prms, traits, mean.abund, sd.abund, w.evo){
  ...
  interact.sample <- function(abundance, trait.overlap, nplant,
    ...
    samp.2468 <- function(fill, nplant, trait.abund){
      samps <- function(prob){
        samp <- matrix(rmultinom(1, fill*prob, trait.abund),nrow=nplant)
      }
      samp2 <- samps(0.2)
      samp4 <- samps(0.4)
      samp6 <- samps(0.6)
      samp8 <- samps(0.8)
      return(list(samp2=samp2, samp4=samp4, samp6=samp6, samp8=samp8))
    }
    
    trait <- matrix(trait.overlap, nrow=nplant)
    trait0 <- c(list(evo= matrix(0), eco= matrix(0),
    eco2= matrix(0)), rep(list(samp2=matrix(0), samp4=matrix(0),
                         samp6=matrix(0), samp8=matrix(0)), 2))

    if(any(trait !=0)){
      trait <- exp(log(trait) - log(sum(trait)) + log(100*w.evo)) 
      abundance <- exp(log(abundance) - log(sum(abundance)) + log(100))
      ## ecological matrix with random abundances
      trait.abund <- trait*abundance
      trait.abund <-  exp(log(trait.abund) -log(sum(trait.abund)) + log(100)) 
      ...
      if(fill.trait.abund !=0 & fill.trait.abund2 !=0 &
        ...
        abund2.samp <- samp.2468(fill.trait.abund2, nplant,
                                 trait.abund2)
        return(c(list(evo = trait, eco = trait.abund, eco2 =
                      trait.abund2), abund.samp, abund2.samp))
      }else{
        return(trait0)
      }
    }else{
  ...
  sim.abund <- function(nanimal, nplant, mean.dist, sd.dist){
    abund.a <- rlnorm(nanimal, mean.dist, sd.dist)
    abund.p <- rlnorm(nplant, mean.dist, sd.dist)
