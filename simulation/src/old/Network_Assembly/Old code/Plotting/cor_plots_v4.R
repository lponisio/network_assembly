...
cor.plots <- function(xdatas, ydata1s, ydata2s, ydata3s, trait){ 
	cols <- c(hsv(0.5, s=0.5, alpha=0.4), hsv(0.1, s=0.5, alpha=0.4), hsv(1, s=0.5, alpha=0.4))
	...
	###first trait
		for(j in 1:length(trait)){
			...
			if(i == length(xdatas)){
				axis(side=1)
				mtext("Nestedness Ecological", side=1, line=2.5, cex=0.55)
			}
			
			if(j == 1){
				axis(side=2)
				mtext("Nestedness fundamental and sampling", side=2, line=2.5, cex=0.55)
			}
			...
			if(i == 1 & j == 3){
				legend("topleft", legend=c("Fundamental", "Good sampling", "Poor sampling"), col=cols, pch=16, bty="n")	
			}	##close if
		}	## close j loop 
	} ## close i loop	
	...
	for(i in 1:length(xdatas)){
		...
		if(i==1){
			...
			if(i == length(xdatas)){
				axis(side=1)
				mtext("Modularity ecological", side=1, line=2.5, cex=0.55)
			}
			
			if(j == 1){
				axis(side=2)
				mtext("Modularity fundamental and sampling", side=2, line=2.5, cex=0.55)
			}
