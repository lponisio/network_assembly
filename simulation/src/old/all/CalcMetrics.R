...
calc.metric <- function(data) {
  ...
  calc.mod <- function(data){ 
    ...
    mod.met <- try(modularity(graph,
                              ...
                              weights=weights, directed= FALSE), silent=TRUE)
  ...
  }else if(all(data[data !=0] == 1)){ # matrix all 1s and 0s
    nodf <- nestednodf(data, weighted=FALSE)$statistic["NODF"]
    mets <- c(nodf,
              try(networklevel(data, index=network.metrics[-1],
                               H2_integer=TRUE), silent=TRUE))
  }else{ # weighted matrix
    mets <- try(networklevel(data, index=network.metrics,
...
network.metrics <- function (data, N) {
  ...
  zvals <-function(stats){
    z.sd <- (stats[,1] -
             apply(stats[,-1], 1, mean, na.rm = TRUE))/
               apply(stats[,-1], 1, sd, na.rm = TRUE)
