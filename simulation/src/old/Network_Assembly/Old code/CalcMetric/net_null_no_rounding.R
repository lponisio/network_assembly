...
calc.metric <- function(data) {
	...
	mets <- try(networklevel(data, index=c("weighted NODF")), silent=TRUE)
	if(class(mets) == "try-error") {
	  mets <- NA
	}
...
null.stat <- function(prob,fill,nr,nc) {
	...
	####following line is only necessary when using a null model that does not constraint marginals, i.e. prob.null2
		while(any(dim(sim.web) < 10) | is.matrix(sim.web) == FALSE) sim.web <- prob.null2(prob,fill,nr,nc) 
	...
	## check to make sure the rounded,emptied matrix is large enough to calculate statistics on
    	...
    	#print(data)
			...
			##compute p-values
				...
				pval.mod <- sum(out.mets[2,-1] >= out.mets[2,1], na.rm=TRUE)/(N+1)
			
		
				return(c(nodf = out.mets[1,1], z.nodf=z.nodf, pval.nodf=pval.nodf,
					mod = out.mets[2,1], z.mod = z.mod, pval.mod = pval.mod)) 
