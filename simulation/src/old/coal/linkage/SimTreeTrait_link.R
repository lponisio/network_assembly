...
make.pa.mat <- function(prms, traits, mean.abund, sd.abund, w.evo){
  ...
  interact.sample <- function(abundance, trait.overlap, nplant, w.evo){
    ...
    return(list(evo = trait, eco = trait.abund, eco2 = trait.abund2))
  }

  ##simulate abundances and define interaction probability 
  sim.abund <- function(nanimal, nplant, mean.dist, sd.dist){
    abund.a <- rpoilog(S = nanimal, mu = mean.dist, sig= sd.dist, keep0 =TRUE)
...
interact.phylo.signal <- function(dist.tree, int.mat) {
  ...
  if(sum(int.mat) != 0 & all(is.na(int.mat) == FALSE)){
    ...
    if(is.matrix(int.mat)){
      ...
      if(all(dim(int.mat) >= 5) & any(int.mat!=1)) {
...
link.rule <- function(prms, traits, nnull, meanAbund, sdAbund, w.evo) {
  ...
  link.dist <- function(linkages){
    means <- sapply(linkages, mean, na.rm=TRUE)
    vars <- sapply(linkages, var, na.rm=TRUE)
    kt <- sapply(linkages, kurtosis, na.rm=TRUE)
    skew <- sapply(linkages, skewness, na.rm=TRUE)
    return(rbind(means, vars, kt, skew))  
  }

  link.pol <- sapply(empty.interact, colSums)
  link.plant <- sapply(empty.interact, rowSums)

  char.pol <- link.dist(link.pol)
  char.plant <- link.dist(link.plant)
  
  out.dist <- sapply(empty.interact, function(x){
    if(sum(x) != 0 & all(dim(x) >5)){
      dists <- degreedistr(x, plot.it=FALSE, silent=TRUE)
      higher <- dists[[1]][,"AIC"]
      lower <- dists[[2]][,"AIC"]
  ...
  return(rbind(out.metrics, out.cors, out.fill,
               out.npol, out.nplant, char.pol, char.plant, out.dist))
}
