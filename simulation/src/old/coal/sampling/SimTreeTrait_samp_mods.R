...
make.pa.mat <- function(prms, traits, mean.abund, sd.abund, w.evo){
  ...
  interact.sample <- function(abundance, trait.overlap, nplant,
      ...
      trait0 <- c(rep(list(samp2=matrix(0), samp4=matrix(0),
      ...
      if(any(trait !=0)){
        ...
        if(fill.trait.abund !=0 & fill.trait.abund2 !=0 &
          ...
          abund2.samp <- samp.2468(fill.trait.abund2, nplant, trait.abund2)
          
          return(c(abund.samp, abund2.samp))
        }else{
          return(trait0)
        }
      }else{
  ...
  sim.abund <- function(nanimal, nplant, mean.dist, sd.dist){
    ...
    abund.p <- rpoilog(S = nplant, mu = mean.dist, sig= sd.dist, keep0=TRUE)
...
link.rule <- function(prms, traits, nnull, meanAbund, sdAbund, w.evo) {
  ...
  rownames(out.metrics) <- c("NODF", "H2", "modularity.g",
                             "modularity.r", "modularity.d", "z.NODF",
                             "z.H2", "z.mod.g", "z.mod.r", "z.mod.d",
                             "z2.NODF", "z2.H2", "z2.mod.g",
                             "z2.mod.r", "z2.mod.d", "p.NODF", "p.H2",
                             "p.mod.g", "p.mod.r", "p.mod.d")
