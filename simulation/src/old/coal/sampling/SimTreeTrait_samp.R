...
make.pa.mat <- function(prms, traits, mean.abund, sd.abund, w.evo){
  ...
  interact.sample <- function(abundance, trait.overlap, nplant, w.evo){
    ...
    fill.trait.abund  <- sum(trait.abund)
    fill.trait.abund2 <- sum(trait.abund2)

    if(fill.trait.abund !=0 & fill.trait.abund2 !=0){
      trait.abund.samp.1 <- matrix(rmultinom(1,
                                             1000*0.1,
                                             trait.abund), nrow=nplant)
      trait.abund2.samp.1 <- matrix(rmultinom(1,
                                              1000*0.1,
                                              trait.abund2), nrow=nplant)
      trait.abund.samp.5 <- matrix(rmultinom(1,
                                             1000*0.5,
                                             trait.abund), nrow=nplant)
      trait.abund2.samp.5 <- matrix(rmultinom(1,
                                              1000*0.5,
                                              trait.abund2), nrow=nplant)

      
      return(list(abund.samp.1 = trait.abund.samp.1,
                  abund2.samp.1 = trait.abund2.samp.1,
                  abund.samp.5 = trait.abund.samp.5,
                  abund2.samp.5 = trait.abund2.samp.5))
    } else{
      return(list(abund.samp.1 = 0,
                  abund2.samp.1 =0,
                  abund.samp.5 = 0,
                  abund2.samp.5 = 0))
    }
