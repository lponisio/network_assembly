...
network.metrics <- function (data, N) {
	...
	## check to make sure the rounded,emptied matrix is large enough to calculate statistics on
    	...
    	print(data)
			...
			z.ISA <- (out.mets[2,1] - mean(out.mets[2,-1],na.rm=TRUE))/sd(out.mets[2,-1],na.rm=TRUE)
			z.h2 <- (out.mets[3,1] - mean(out.mets[3,-1],na.rm=TRUE))/sd(out.mets[3,-1],na.rm=TRUE)
		
			return(c(nodf = out.mets[1,1], z.nodf=z.nodf,
					ISA = out.mets[2,1], z.ISA = z.ISA, 
					h2 = out.mets[3,1], z.h2 = z.h2 )) 
