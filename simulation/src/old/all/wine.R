wine.2 <- function (web, nreps = 1) {
  wincomp <- function(M, Nr, Nc, Nl, xi, yj) {
    M.packed <- M[order(rowSums(M > 0)), order(colSums(M > 
                                                       0)), drop = FALSE]
    Pc <- M.packed/matrix(rep(rowSums(M.packed), Nc), Nr, 
                          Nc)
    Pr <- M.packed/matrix(rep(colSums(M.packed), Nr), Nr, 
                          Nc, byrow = TRUE)
    dij.w <- Pr * matrix(rep(xi, Nc), Nr, Nc) + Pc * matrix(rep(yj, 
                                                                Nr),
                                                            Nr,
                                                            Nc,
                                                            byrow = TRUE)
  ...
  shuffle.equiprob <- function(M) {
    ...
    if (!identical(dim(M)[1], dim(M)[2])) {
      ## are there less rows or columns
      dims <- which(dim(M) == min(dim(M)))
      ## and how many species is that
      vdims <- dim(M)[dims]
      ## are there more rows of columns
    ...
    lMR <- vdims * vdimb
    ## sample row/colmns (which ever was min dim) 
    sample1 <- sample(vdimb, vdims)
    ## shuffle interactions
    diag1 <- randiag(vdims)
    MR[, sample1] <- diag1
    ## sample the now/columns (which ever was mas dim) except those
    ## already drawn by sample 1
  ...
  xi <- (i - 1)/Nr + 1/(2 * Nr)
  yj <- (j - 1)/Nc + 1/(2 * Nc)
  dw <- wincomp(M = M, Nr = Nr, Nc = Nc, Nl = Nl, xi = xi, 
                yj = yj)
  dij.w <- dw$dij.w
  dw <- dw$win
  dmax <- wincomp(M = mpack(M), Nr = Nr, Nc = Nc, Nl = Nl, 
                  xi = xi, yj = yj)
  dij.max <- dmax$dij.w
  dmax <- dmax$win
  d.rnd <- NULL
  for (i in 1:nreps) {
    ...
    d.rnd <- c(d.rnd, wincomp(M = MR, Nr = Nr, Nc = Nc, Nl = Nl, 
                              xi = xi, yj = yj)$win)
  }
  z_score <- (dw - mean(d.rnd))/sd(d.rnd)
  p_value <- 1 - pnorm(dw, mean(d.rnd, na.rm = TRUE), sd(d.rnd, 
                                         na.rm = TRUE))
  eta.w <- (dw - mean(d.rnd))/(dmax - mean(d.rnd))
  result <- list(win = dw, wine = eta.w, zscore = z_score, 
                 pvalue = p_value, dmax = dmax, drnd = mean(d.rnd),
                 dij.w = dij.w, 
                 dij.max = dij.max)
