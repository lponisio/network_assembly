##load data with sampling, no evo, eco

load.fix.samp <- function(files, paths, nrule, nmat, ntopo){
  ...
  mats <- rep(c("abund.samp.2",
                ...
                "abund2.samp.4",
                "abund2.samp.6",
                "abund2.samp.8"), nsim*nrule*ntopo)
...
cats[,"link.rule"][grep("matching", cats[,"link.rule"])] <- "matching"
