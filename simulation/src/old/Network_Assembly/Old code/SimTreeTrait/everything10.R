...
character.evo <- function(tree1, tree2){
  ...
  optima.tree2 <- c(sample.trait,ace(x=sample.trait, phy=tree2)$ace)
  optima.tree2 <- optima.tree2[tree2$edge[,2]]
  tree2.trait2 <- exp(rTraitCont(tree2, model="OU", theta=optima.tree2))
